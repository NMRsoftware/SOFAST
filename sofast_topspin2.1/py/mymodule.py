...
def cdec_bilvl():
   ...
   #cmd = "pldb30 %f" % (pl12);   XCMD(cmd);
   #cmd = "pldb31 %f" % (pl12-3); XCMD(cmd);
   pcpd2=float(GETPAR("PCPD 2"))
   pl12 =float(GETPAR("PL 12"))
   
   cmd = "pcpd2 %f" % (pcpd2*2);   XCMD(cmd);

   cmd = "pl12 %f" % (pl12+6);   XCMD(cmd);
   cmd = "pl30 %f" % (pl12+6);   XCMD(cmd);
   cmd = "pl31 %f" % (pl12+6-3); XCMD(cmd);
...
def cdec_garp():		#for 13C-decoupling of methyl
   ...
   cmd = "pl12 %f" % (pl12+6);   XCMD(cmd);  #increase decoupling power level by 4 times
